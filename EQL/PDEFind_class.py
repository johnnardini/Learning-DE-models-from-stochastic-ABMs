...
class PDE_Findclass(object):
    ...
    def train_val_ODEFind_OOS(self,div_factor):
        ...
        for i in np.arange(self.deg + 1):
            Theta[:,i] = U**i
        description = ['u^'+str(i) for i in np.arange(self.deg+1)]
        description[0] = '1'
    ...
    def logistic_compare(self):
        ...
        '''n=50
        T = mat[0]['variables'][0][:n]
        U = mat[0]['variables'][1][:n,0]
        Ut = [mat[0]['variables'][2][:n]]
        F = mat[0]['F'][:n,0]'''
        ...
        for r in np.arange(self.reals):
            for i,d in enumerate(self.data_dir):
                ...
                '''#### testing
                xi_true = np.array([0,0,.1])[:,np.newaxis]

                res_true = Ut[0] - np.matmul(Theta,xi_true)
                res_learned = Ut[0] - np.matmul(Theta,xi)

                plt.plot(res_true,'.',label='True')
                plt.plot(res_learned,'.',label='learned')
                plt.show()
    ...
    def logistic_compare_2(self,norm = 0):
        ...
        T = mat[0]['variables'][0]
        if norm == 1:
            U_max = np.max(mat[0]['variables'][1][:,0])
            U = mat[0]['variables'][1][:,0]/U_max
            Ut = [mat[0]['variables'][2]/U_max]
        else:
            U = mat[0]['variables'][1][:,0]
            Ut = [mat[0]['variables'][2]]
        F = mat[0]['F'][:,0]
        ...
        self.description1 = ['u','u(1-u)']
        self.description2 = ['u','u(1-Fu)']

        for r in np.arange(self.reals):
            utTrain,theta1Train,ptrain,utVal,theta1Val,pval,utTest,theta1Test,ptest = data_shuf(Ut[i],Theta1,self.shuf_method,self.trainPerc,self.valPerc,len(x),len(t))
            theta2Train = Theta2[ptrain,:]
            theta2Val = Theta2[pval,:]
            theta2Test = Theta2[ptest,:]


            xi1, hparams1, val_score1,num_nonzero_u1 = run_PDE_Find_train_val(theta1Train,utTrain,theta1Val,utVal,self.algo_name,self.description1,lambda_lb=-3,lambda_ub=-1,deriv_list=self.deriv_index[i])

            xi2, hparams2, val_score2,num_nonzero_u2 = run_PDE_Find_train_val(theta2Train,utTrain,theta2Val,utVal,self.algo_name,self.description2,lambda_lb=-3,lambda_ub=-1,deriv_list=self.deriv_index[i])
            

            testing_err1 = np.linalg.norm(Ut[0][ptest]-np.matmul(theta1Test,xi1))
            testing_err2 = np.linalg.norm(Ut[0][ptest]-np.matmul(theta2Test,xi2))
    ...
    def train_val_PDEFind_reduce_library(self):
        ...
        #load in library and du/dt
        X,T,Ut,Theta,description = theta_construct_two_subpops_reduce_library(mat,self.deg)
        ...
        #rel_ind = [np.abs(Ut[i])>1e-3 for i in np.arange(len(self.data_dir))]
        #Ut = [Ut[i][rel_ind[i]] for i in np.arange(len(self.data_dir))]
        #Theta = [Theta[np.squeeze(rel_ind[i]),:] for i in np.arange(len(self.data_dir))]
        #X = [X[rel_ind[i],np.newaxis] for i in np.arange(len(self.data_dir))]
        #T = [T[rel_ind[i],np.newaxis] for i in np.arange(len(self.data_dir))]
        ...
        for r in np.arange(self.reals):
            for i,d in enumerate(self.data_dir):
                ...
                #shuffle ut values into train-val data
                if self.shuf_method == 'neighbors_1d':
                    utTrain,thetaTrain,ptrain,utVal,thetaVal,pval = data_shuf_1d(Ut[i],Theta[i],self.trainPerc,self.valPerc,X,T,N_neighbors = 9)

                else:    
                    utTrain,thetaTrain,ptrain,utVal,thetaVal,pval,_,_,_ = data_shuf(Ut[i],Theta[i],self.shuf_method,self.trainPerc,self.valPerc,len(x),len(t))
                ...
                #print "Predicted equation with full library is " + print_pde(xi,self.description[i],ut=data_description[i]+'_{t}')
                
                
                #keep xi values that either were not selected or 
                #had significant values
                keep_ind = (xi==0)+(np.abs(xi)>=0.05)
                Theta_red = Theta[i][:,np.squeeze(keep_ind)]
                description_red = [self.description[i][j]  for j in np.where(np.squeeze(keep_ind!=0))[0]]
                
                if self.shuf_method == 'neighbors_1d':
                    utTrain,thetaTrain,ptrain,utVal,thetaVal,pval = data_shuf_1d(Ut[i],Theta_red,self.trainPerc,self.valPerc,X,T,N_neighbors = 9)
                else:
                    utTrain,thetaTrain,ptrain,utVal,thetaVal,pval,_,_,_ = data_shuf(Ut[i],Theta_red,self.shuf_method,self.trainPerc,self.valPerc,len(x),len(t))               
                #

                
                #train and validate PDE-FIND again
                xi, hparams, val_score,num_nonzero_u = run_PDE_Find_train_val(thetaTrain,
                                            utTrain,
                                            thetaVal,
                                            utVal,
                ...
                if len(xi[xi!=0]) > 1:
                    ...
                    #don't prune if xi only has one nonzero vector
                    xi_new = xi

                #threshold
                xi[np.abs(xi)<.01] = 0
                xi_new[np.abs(xi_new)<.01] = 0
                    
                if self.print_pdes == True:
                    print("Predicted equation is " + print_pde(xi_new,description_red,ut=data_description[i]+'_{t}'))

                    
                xi_list_no_prune[i].append(xi_convert_full(xi,
                                                           description_red,
                                                           self.description[i]))
                
                xi_list[i].append(xi_convert_full(xi_new,
                                                  description_red,
                                                  self.description[i]))
    ...
    def list_common_eqns(self):
        ...
        data_description = ["C"]#[chr(117+i) for i in np.arange(len(self.data_dir))]
        ...
        for i,d in enumerate(self.data_dir):
            if os.path.isfile(self.write_dir[i]+".npz"):
                ...
                #looking into all learned Eqns.
                #x = np.zeros((3,1))
                #for j in range(len(data['xi_list'])): x = np.hstack((x,data['xi_list'][j]))
                #print np.mean(x[:,1:],axis=1)
                #print np.std(x[:,1:],axis=1)
                
        
                xi_vote_tmp = []

                for j in range(len(data['xi_list'])):
                    xi_vote_tmp.append(trans_rev((np.abs(data['xi_list'][j]) > 1e-4)*1)[0])
        ...
        print(print_pde(xi_vote_params[0][0][:,np.newaxis],data['description']))
    ...
    def list_common_eqns_SIR(self):
        ...
        xi_vote = [[] for d in np.arange(len(data['xi_list']))]
        xi_vote_params = [[] for d in np.arange(len(data['xi_list']))]
        xi_vote_params_SD = [[] for d in np.arange(len(data['xi_list']))]
